...
citiesRouter
  .get('/', async (req, res, next) => {
    ...
    const cities = await db.any(
      'SELECT * FROM cities ORDER BY country, province, city'
    );
  ...
  .post('/', async (req, res, next) => {
    try {
      const { city, province, country, zip } = req.body;
      const id = uuidv4();
      const created = await db.one(
        `INSERT INTO cities (id, city, province, country, zip) VALUES ($1, $2, $3, $4, $5) RETURNING id, city, province, country, zip`,
...
citiesRouter.get('/:limit/:offset', async (req, res, next) => {
  try {
    ...
    const cities = await db.any(
      'SELECT * FROM cities ORDER BY country, province, city LIMIT $1 OFFSET $2',
      [limit, offset]
    );
    return res.send(cities);
  } catch (e) {
